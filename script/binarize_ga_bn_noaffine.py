...
class BinarizedNeuroEvo(nn.Module):
    def __init__(self):
        ...
        # 移動平均は無視，学習可能パラメータなしで正規化のみ
        self.bn1 = nn.BatchNorm1d(128, track_running_stats=False, affine=False)
...
def genetic_algorithm(model, x_eval, y_eval):
    ...
    bias = 10   # fc2.biasのみ（BNはaffine=Falseなのでパラメータなし）
...
def main():
    ...
    model.fc2.bias.data = best_b.clone()
    ...
    plt.ylabel("Accuracy")
    plt.title("Accuracy History (BN, affine=False)")
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig("./output/binarize_ga_bn_noaffine.png")

    np.save("./output/acc_bn_noaffine.npy", np.array(acc_history))
    np.save("./output/loss_bn_noaffine.npy", np.array(loss_history))

    print("グラフを ./output/binarize_ga_bn_noaffine.png に保存した．")
